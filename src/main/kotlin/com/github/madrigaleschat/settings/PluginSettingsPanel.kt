...
class PluginSettingsPanel {
    ...
    private val warningLabel =
        JBLabel("All events are OFF — plugin will not publish anything").apply {
    ...
    private val tableModel =
        object : DefaultTableModel(arrayOf("Event", "Mode"), eventNames.size) {
    ...
    init {
        ...
        topicPrefixField.document.addDocumentListener(
    ...
    private fun setAllModes(mode: EventMode) {
        ...
        eventNames.forEachIndexed { i, name ->
            val effective =
                if (mode == EventMode.REDACTED && name in PluginSettings.FULL_ONLY_EVENTS) {
                    EventMode.FULL
                } else {
                    mode
                }
